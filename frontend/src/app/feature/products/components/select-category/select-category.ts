...
export class SelectCategory implements OnInit {
  ...
  public initialCategory = input<ModelCategory | null>();
  ...
  constructor() {
    effect(() => {
      ...
      if (initial && !this.categories().some(c => c.id === initial.id)) {
        this.categories.update(current => [initial, ...current]);
      }
    });
  }
    ...
    this.categoryService.getCategories(page, 5).subscribe({
      next: (response) => {
        this.categories.update(current => {
          // Filtramos los duplicados para evitar choques con el initialCategory
          const newCategories = response.content.filter(
            (newCat: ModelCategory) => !current.some(existingCat => existingCat.id === newCat.id)
          );
